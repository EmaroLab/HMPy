...
class Classifier:
    ...
    def classify(self,model, dtype, feature_extraction):
        ...
        if dtype == "3IMU_acc":
            if self.feature_extraction==True:
                ...
                self.window_size,m = self.model.component["acc"][0].shape
            ...
            if (numWritten >= self.window_size):
                if self.dtype == "3IMU_acc":
                    if self.feature_extraction == True:
                        ...
                        possibilities_vector[i,0] = self.compareModel(self.model,acc,self.window_size);
    ...
    def compareModel_f(self,model,gravity,body,window_size):
        """
    ...
    def compareModel(self,model,acc,window_size):
        """
        Compare model with a window of data sensory
        ...
        :param window_size: integer that indicate the size of the window
        """
        distanceA = self.compareComponent(model,"acc",acc,window_size, "mh")
        overall = distanceA
